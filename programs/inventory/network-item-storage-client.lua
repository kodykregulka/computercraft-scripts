...
function storageSystem.chestGroups.findNextChestSlotWithCriteria(chestList, criteriaFunc)
	...
	repeat
		...
		local slotIndex, itemCount, itemName = storageSystem.chestGroups.findNextSlotWithCriteria(chest.pWrap,
			storageSystem.chestGroups.nextSlot(chest.lastSlot, chest.length), criteriaFunc)
		if (slotIndex > 0)
		then
...
function storageSystem.chestGroups.dropIntoChestList(chestList, sourceChestObj, sourceSlot, desiredAmount)
	...
	repeat
		local transferedAmount = transferedAmount +
		chestList.get(chestIndex).pWrap.pullItems(sourceChestObj.name, sourceSlot, desiredAmount - transferedAmount)
...
function storageSystem.comms.sendAndReceive(targetID, message, protocol, timeout, wantWarnings)
	...
	elseif (message.code == 400) then
		return false, {},
			"Bad request: " .. message.code .. " : " .. (message.data.errorMessage or textutils.serialize(message))
	elseif (message.code == 500) then
		return false, {},
			"Internal server error: " ..
			message.code .. " : " .. (message.data.errorMessage or textutils.serialize(message))
	elseif (not wantWarnings and message.code == 206) then
		return false, {},
			"Unable to fully process the request: " ..
			message.code .. (message.data.errorMessage or textutils.serialize(message))
	else
...
function storageSystem.comms.sendGetClientProtocol()
	local success, data, errorMessage = storageSystem.comms.sendAndReceive(storageSystem.comms.serverID,
		{ command = "getClientProtocol" }, storageSystem.config.data.serverProtocol, storageSystem.config.data.timeout,
		false)
...
function storageSystem.comms.sendGetClientList()
	local success, data, errorMessage = storageSystem.comms.sendAndReceive(storageSystem.comms.serverID,
		{ command = "getClientList" }, storageSystem.config.data.serverProtocol, storageSystem.config.data.timeout, false)
...
function storageSystem.comms.sendRegisterClientName(clientName)
	local success, data, errorMessage = storageSystem.comms.sendAndReceive(storageSystem.comms.serverID,
		{ command = "registerClientName", clientName = clientName }, storageSystem.config.data.serverProtocol,
		storageSystem.config.data.timeout, false)
	if (success) then
...
function storageSystem.comms.sendRegisterClientChests()
	...
	data.inputGroup = storageSystem.chestGroups.pnetwork.config.groupList
	[storageSystem.chestGroups.getClientInputGroupName()]
	data.outputGroup = storageSystem.chestGroups.pnetwork.config.groupList
	[storageSystem.chestGroups.getClientOutputGroupName()]
	local success, data, errorMessage = storageSystem.comms.sendAndReceive(storageSystem.comms.serverID, data,
		storageSystem.config.data.serverProtocol, storageSystem.config.data.timeout, false)
	if (success) then
		return true
	else
...
function storageSystem.comms.sendGetPnetwork()
	local success, data, errorMessage = storageSystem.comms.sendAndReceive(storageSystem.comms.serverID,
		{ command = "getPnetwork" }, storageSystem.config.data.serverProtocol, storageSystem.config.data.timeout, false)
...
function storageSystem.setupClientChests()
	...
	print(
	"you need to assign the peripherials/chests to this client's input group (i), its output group (o), or leave blank to not assign to a group")

	local success, errorMessage = storageSystem.chestGroups.pnetwork.createGroup(storageSystem.chestGroups
	.getClientInputGroupName())
	if (not success) then return false, "Client input group failed to create due to: " .. errorMessage end
	local success, errorMessage = storageSystem.chestGroups.pnetwork.createGroup(storageSystem.chestGroups
	.getClientOutputGroupName())
	...
	for pName, pObject in pairs(storageSystem.chestGroups.pnetwork.newPList) do
		...
		elseif (groupName == "input" or groupName == "i") then
			...
			storageSystem.chestGroups.pnetwork.addNewPeripheralToGroup(pObject,
				storageSystem.chestGroups.pnetwork.config.groupList[storageSystem.chestGroups.getClientInputGroupName()])
		elseif (groupName == "output" or groupName == "o") then
			print("adding to output group")
			storageSystem.chestGroups.pnetwork.addNewPeripheralToGroup(pObject,
				storageSystem.chestGroups.pnetwork.config.groupList
				[storageSystem.chestGroups.getClientOutputGroupName()])
		else
	...
	storageSystem.chestGroups.clientInputChestGroup = storageSystem.chestGroups.createChestGroup(storageSystem
	.chestGroups.getClientInputGroupName())
	storageSystem.chestGroups.clientOutputChestGroup = storageSystem.chestGroups.createChestGroup(storageSystem
	.chestGroups.getClientOutputGroupName())
...
function storageSystem.setupNewClient()
	...
	local success, serverLookupNameIDMap, errorMessage = storageSystem.comms.lookupHostNames(storageSystem.config.data
	.serverProtocol)
	if (not success) then
...
if (fs.exists(storageSystem.config.getResolvedFileName())) then
	...
	storageSystem.comms.serverID = rednet.lookup(storageSystem.config.data.serverProtocol,
		storageSystem.config.data.serverName)
else
...
local success, errorMessage = storageSystem.chestGroups.pnetwork.loadConfigFile(storageSystem.chestGroups.pnetwork
.configFileName)
if (not success) then
...
storageSystem.chestGroups.serverInputChestGroup = storageSystem.chestGroups.createChestGroup(storageSystem.chestGroups
.getServerInputGroupName())
storageSystem.chestGroups.clientInputChestGroup = storageSystem.chestGroups.createChestGroup(storageSystem.chestGroups
.getClientInputGroupName())
storageSystem.chestGroups.clientOutputChestGroup = storageSystem.chestGroups.createChestGroup(storageSystem.chestGroups
.getClientOutputGroupName())
...
local function handle_put(args)
	...
	while (storageSystem.chestGroups.checkIfChestContainsItems(storageSystem.chestGroups.clientInputChestGroup)) do
		storageSystem.chestGroups.moveAllItemsInGroup(storageSystem.chestGroups.clientInputChestGroup,
			storageSystem.chestGroups.serverInputChestGroup)
	end
end
...
local function handle_get(args)
	...
	storageSystem.comms.sendAndReceive(storageSystem.comms.serverID, dataMessage,
		storageSystem.config.data.clientProtocol, storageSystem.config.data.timeout)
end
