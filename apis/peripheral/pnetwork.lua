...
pnetwork.configFileName = ""

function pnetwork.loadConfigFile(fileName)
    pnetwork.config = {}
    ...
    then
        pnetwork.configFileName = fileName
        return true
    else
...
function pnetwork.saveConfigFile(configFileName)
    ...
    if(not pnetwork.configFileName) then
        error("no config file name provided to save to", 2)
    end

    local configFile = fs.open(pnetwork.configFileName, "w")
...
function pnetwork.createGroup(groupName)
    ...
    pnetwork.config.groupList[groupName].members = {}
    return true
end
...
function pnetwork.renameGroup(currentGroupObj, newGroupName)
    ...
    for key, value in pairs(currentGroupObj.members) do
...
function pnetwork.removeGroup(groupName)
    ...
    for key, value in pairs(pnetwork.config.groupList[groupName].members) do
...
function pnetwork.addNewPeripheralToGroup(pObject, groupObj)
    ...
    groupObj.members[pObject.name] = pObject
...
function pnetwork.removeFromGroup(pName, groupObj)
    ...
    if(not groupObj.members[pName])then
        return false, "peripheral not found with that name in that group"
    end

    local pObject = groupObj.members[pName]
    pObject.group = nil
    pnetwork.pList[pObject.name] = nil
    pnetwork.newPList[pObject.name] = pObject
    return true
end
...
function pnetwork.ui.command.save()
    if(pnetwork.configFileName == "")
    then
        print("saving to ./" .. pnetwork.DEFAULT_CONFIG_FILENAME .. ", please move and rename on your own")
        pnetwork.configFileName = shell.resolve(pnetwork.DEFAULT_CONFIG_FILENAME)
    else
        print("Overwriting previous save file at: " .. pnetwork.configFileName)
    end
    
    pnetwork.saveConfigFile(pnetwork.configFileName)
...
function pnetwork.ui.command.assignNewPeripherals()
    ...
    for pName, pObject in pairs(pnetwork.newPList) do
        ...
        then
            ...
            elseif(pnetwork.config.groupList[groupName].members[pObject.name])
            then
...
function pnetwork.ui.command.printPeripheral()
    ...
    for name, pObject in pairs(currentGroup.members)
    do
    ...
    if(pName == "")then
        print("not a valid input")
        return
    elseif(not currentGroup.members[pName])then
        print("peripheral not found with that name in that group")
        return
    end
    local pObject = currentGroup.members[pName]
...
function pnetwork.ui.launch(configFileName)
    ...
    pnetwork.configFileName = shell.resolve(pnetwork.configFileName)
    if(not fs.exists(pnetwork.configFileName)) then
        local configFile = fs.open(pnetwork.configFileName, "w")
        configFile.write(textutils.serialize(pnetwork.config))
        configFile.close()
    elseif(pnetwork.loadConfigFile(pnetwork.configFileName))
    then
        print("successfully loaded pnetwork.config file from: " .. pnetwork.configFileName)
    else
        error("Unable to read pnetwork.config file: " .. pnetwork.configFileName)
    end
